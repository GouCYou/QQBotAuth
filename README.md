...
启用 `player.force-verification` 时，未验证聊天由 Paper 的当前签名聊天事件阻止。关闭强制验证时，未绑定玩家可以进入子服，公开聊天及聊天类命令共享默认 30 秒冷却；生存服上的纯正整数输入会绕过代理冷却，供 QuickShop 接收购买数量。移动和交互限制仍可覆盖背包点击、拖动、物品拾取、丢弃、切换、食用、书本编辑、攻击、钓鱼、桶和盔甲架操作；玩家同时不会受到伤害或掉饥饿值。
